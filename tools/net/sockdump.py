...
def parse_event(event, size):
    ...
    data_type = ct.c_ubyte * data_len
    data = bytes(ct.cast(event, ct.POINTER(data_type)).contents)
...
def pcap_output(cpu, event, size, hdr=True):
    global flush_after_each_packet
...
def main(args):
    ...
    if args.format == 'pcap' and args.raw:
        b['events'].open_perf_buffer(lambda cpu, event, size: output_fn(cpu, event, size, hdr=False), page_cnt=npages)
    else:
